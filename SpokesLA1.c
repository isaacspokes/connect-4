...
void startgame(char *board, int *i){ //beginning of game
    char let;
    printf("\nOpening notes:\n\nRed player uses O's, Yellow player uses X's. \nPlease do not attempt to load a game that has been won.\nIf you enter a number with more than one digit such as \"465\" the code will run through multiple players' turns.\n\nCONNECT FOUR\n\nHello! Enter L to load an old game, N for a new one: ");
...
void displayboard(char board[]){ 
	/* This function opens the currentgame file and writes to it
	the current game board, using the board array. It then prints 
	out the game to the terminal, checking the values of c in 
	order to print out the correct color if it's an R or a Y, then
	printing those as O or X */
    char c;
    FILE *ifile, *ofile;
    ...
    while (fscanf(ofile, "%c", &c) != EOF){
        if(c == 'R'){
            printf("\033[22;31mO"); //prints R with red O
        } else if (c == 'Y'){
            printf("\033[01;33mX"); //prints Y with yellow X
        } else {
            printf("\033[22;34m%c", c); //prints everything else blue
        }
    }
}
...
void userinput(char *board, int x, int *y){
    ...
    if (x % 2 == 0){ //Red player
    	...
    	while (!leg && *y!=1){ //illegal move, asks again
       		printf("\033[22;31m\nIllegal move, pick another column: \n");
        	scanf(" %c", &m1);
        	leg=legalmove(board, m1, &*y);
    	}
    	plyr = x+1;
...
int gamewon(char board[]){
	...
	//this is the diagonal check, looks in both diagonal directions
	for (i=0; i<42; i++){
		if((i%7<4) && (i<18)){ //check from upper left corner
			if((board[i] != 32) && (board[i] == board[i+8]) && (board[i] == board[i+16]) && (board[i] == board[i+24])){
				return(1);
			}
		}
		if((i%7<4) && (i>20)){ //check from lower left corner
...
void winner(int x){ //congratulates the correct winner
    printf("\nGame has ended. \nCongratulations to ");
    if (x % 2 == 1){
        printf("\033[22;31mred player.");
    }
    else {
        printf("\033[01;33myellow player.");
    }
}